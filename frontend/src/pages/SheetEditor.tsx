...
export default function SheetEditor() {
  ...
  const renderFuturesRiskConfirm = () => (
    <div className="space-y-4">
      <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-sm t-danger">
      ...
      {FUTURES_RISK_CONFIRMS.map((text, i) => (
        <label key={i} className="flex items-start gap-3 p-3 t-bg2 rounded-lg border t-border cursor-pointer hover:t-bg3">
          <input type="checkbox" checked={sheet.futuresRiskConfirm[i]}
            ...
            className="mt-0.5 w-5 h-5 rounded border-red-300 t-danger focus:ring-red-500" />
          <span className="text-sm t-text">{text}</span>
        </label>
      ))}
      {!sheet.futuresRiskConfirm.every(Boolean) && <div className="p-3 bg-red-100 rounded-lg text-sm t-danger font-medium">请全部勾选才能继续填写决策表。</div>}
    </div>
  );
  ...
  const renderVeto = () => (
    <div className="space-y-3">
      ...
      {sheet.vetoItems.map((item, i) => (
        <div key={item.id} className={`p-4 rounded-lg border ${item.passed === false ? 'border-red-200 bg-red-50' : item.passed === true ? 'border-green-200 bg-green-50' : 't-border t-bg2'}`}>
          <div className="flex items-start justify-between gap-3">
            <p className="text-sm t-text flex-1">{item.text}</p>
            <div className="flex gap-1.5 flex-shrink-0">
              <button onClick={() => { const items = [...sheet.vetoItems]; items[i] = { ...items[i], passed: true }; save({ vetoItems: items }); }}
                className={`px-3 py-1 rounded text-xs font-medium ${item.passed === true ? 'bg-green-600 text-white' : 't-bg3 t-text2 hover:t-bg3'}`}>是</button>
              <button onClick={() => { const items = [...sheet.vetoItems]; items[i] = { ...items[i], passed: false }; save({ vetoItems: items }); }}
                className={`px-3 py-1 rounded text-xs font-medium ${item.passed === false ? 'bg-red-600 text-white' : 't-bg3 t-text2 hover:t-bg3'}`}>否</button>
            </div>
          </div>
          <input value={item.note} onChange={e => { const items = [...sheet.vetoItems]; items[i] = { ...items[i], note: e.target.value }; save({ vetoItems: items }); }}
            placeholder="备注..." className="mt-2 w-full px-2 py-1 border t-border rounded text-xs focus:outline-none focus:ring-1 focus:ring-blue-500" />
      ...
      <div className={`p-3 rounded-lg text-sm font-medium ${sheet.vetoConclusion === 'passed' ? 'bg-green-100 t-success' : sheet.vetoConclusion === 'failed' ? 'bg-red-100 t-danger' : 't-bg3 t-text2'}`}>
  ...
  const renderQualitative = () => (
    <div className="space-y-3">
      <p className="text-sm t-text2">评分规则：1分很差，3分一般，5分优秀。满分40分。</p>
      {sheet.qualDimensions.map((dim, i) => (
        <div key={dim.id} className="p-4 t-bg2 rounded-lg border t-border">
          <div className="flex items-center justify-between mb-2">
            <div><span className="text-sm font-medium t-text">{dim.name}</span><p className="text-xs t-text2 mt-0.5">{dim.question}</p></div>
            <div className="flex gap-1">{[1, 2, 3, 4, 5].map(s => (
              <button key={s} onClick={() => { const dims = [...sheet.qualDimensions]; dims[i] = { ...dims[i], score: s }; save({ qualDimensions: dims }); }}
                className={`w-8 h-8 rounded-lg text-sm font-bold ${dim.score === s ? 't-accent-bg text-white' : 't-bg3 t-text2 hover:bg-blue-100'}`}>{s}</button>
            ))}</div>
          </div>
          <input value={dim.note} onChange={e => { const dims = [...sheet.qualDimensions]; dims[i] = { ...dims[i], note: e.target.value }; save({ qualDimensions: dims }); }}
            placeholder="备注..." className="t-input w-full text-xs focus:outline-none focus:ring-1 focus:ring-blue-500" />
        </div>
      ))}
      <div className="p-3 bg-blue-50 rounded-lg text-sm font-semibold t-accent">定性小计：{sheet.qualTotal}/40</div>
    </div>
  );
  ...
  const renderQuantitative = () => (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead><tr className="t-bg3">
            <th className="text-left px-3 py-2 font-medium t-text2 border-b">指标</th>
            <th className="text-left px-3 py-2 font-medium t-text2 border-b w-36">近3年/历史</th>
            <th className="text-left px-3 py-2 font-medium t-text2 border-b w-36">当前情况</th>
            <th className="text-center px-3 py-2 font-medium t-text2 border-b w-20">评价</th>
            <th className="text-left px-3 py-2 font-medium t-text2 border-b w-32">备注</th>
          </tr></thead>
          <tbody>{sheet.quantMetrics.map((m, i) => (
            <tr key={m.id} className="border-b t-border">
              <td className="px-3 py-2 t-text font-medium">{m.name}</td>
              <td className="px-3 py-2"><input value={m.historical} onChange={e => { const ms = [...sheet.quantMetrics]; ms[i] = { ...ms[i], historical: e.target.value }; save({ quantMetrics: ms }); }}
                className="t-input w-full text-xs" /></td>
              ...
              <td className="px-3 py-2 text-center">
                <select value={m.rating} onChange={e => { const ms = [...sheet.quantMetrics]; ms[i] = { ...ms[i], rating: e.target.value as typeof m.rating }; save({ quantMetrics: ms }); }}
                  className={`px-2 py-1 border rounded text-xs ${m.rating === 'good' ? 't-success bg-green-50' : m.rating === 'poor' ? 't-danger bg-red-50' : 't-text2'}`}>
      ...
      <div>
        <label className="block text-sm font-medium t-text mb-1">定量分析综合打分（满分30分）</label>
        <input type="number" min="0" max="30" value={sheet.quantScore || ''} onChange={e => save({ quantScore: Math.min(30, Math.max(0, parseInt(e.target.value) || 0)) })}
          className="t-input w-24 font-bold" />
  ...
  const renderValuation = () => (
    <div className="space-y-4">
      <p className="text-sm t-text2">{isFutures ? '计算风险收益比。风险收益比≥2:1才值得做。' : '判断当前价格是否合理，安全边际是否充足。满分20分。'}</p>
      {isFutures ? (
        <div className="space-y-3">
          ...
          {(() => {
            ...
            return (
              <div className={`p-4 rounded-xl border ${ratio >= 2 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                ...
                {ratio < 2 && <p className="text-sm t-danger mt-3 font-medium">⚠️ 风险收益比 &lt; 2:1，建议放弃本次交易。</p>}
                {maxLossPct > 2 && <p className="text-sm t-danger mt-1 font-medium">⚠️ 单笔亏损超过总资金2%上限！</p>}
              </div>
            );
          })()}
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium t-text mb-1">估值判断一句话</label>
            <textarea value={sheet.valuationSummary} onChange={e => save({ valuationSummary: e.target.value })}
              ...
              className="t-input w-full" rows={2} />
          </div>
        </div>
      )}
      <div>
        <label className="block text-sm font-medium t-text mb-1">{isFutures ? '风险收益比打分' : '估值与安全边际打分'}（满分20分）</label>
        <input type="number" min="0" max="20" value={sheet.valuationScore || ''} onChange={e => save({ valuationScore: Math.min(20, Math.max(0, parseInt(e.target.value) || 0)) })}
          className="t-input w-24 font-bold" />
        <span className="text-sm t-muted ml-2">/20</span>
      </div>
    </div>
  );
  ...
  const renderRisks = () => (
    <div className="space-y-4">
      <p className="text-sm t-text2">先写风险，防止只看利好。——芒格"反过来想"</p>
        ...
        <div key={risk.id} className="p-3 t-bg2 rounded-lg border t-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
            ...
            <select value={risk.probability} onChange={e => { const rs = [...sheet.risks]; rs[i] = { ...rs[i], probability: e.target.value as typeof risk.probability }; save({ risks: rs }); }}
              className="t-input text-xs"><option value="">概率</option><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select>
            <select value={risk.impact} onChange={e => { const rs = [...sheet.risks]; rs[i] = { ...rs[i], impact: e.target.value as typeof risk.impact }; save({ risks: rs }); }}
              className="t-input text-xs"><option value="">影响</option><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select>
            <input value={risk.response} onChange={e => { const rs = [...sheet.risks]; rs[i] = { ...rs[i], response: e.target.value }; save({ risks: rs }); }}
              placeholder="应对方式" className="t-input text-xs" />
          </div>
        </div>
      ))}
      <div className="p-4 t-bg3 rounded-lg space-y-2">
        <h4 className="text-sm font-semibold t-text">最坏情况推演</h4>
        {['如果我错了，最可能错在哪', '最坏情况下可能亏损多少', '我能不能承受'].map(q => (
          <div key={q}><label className="text-xs t-text2">{q}</label>
            <input value={sheet.worstCase[q] || ''} onChange={e => save({ worstCase: { ...sheet.worstCase, [q]: e.target.value } })}
              className="t-input w-full text-xs mt-0.5" /></div>
        ))}
      </div>
    </div>
  );
  ...
  const renderLogic = () => (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium t-text mb-1">反方观点：如果我是反对者，我会怎么反驳？</label>
        <textarea value={sheet.counterArgument} onChange={e => save({ counterArgument: e.target.value })}
          className="t-input w-full" rows={3} />
      </div>
      {isFutures && (
        <div>
          <label className="block text-sm font-medium t-text mb-1">我的判断最脆弱的假设是哪一个？</label>
  ...
  const renderScoring = () => {
    ...
    const scoreColor = total >= thresholds.high ? 't-success' : total >= thresholds.mid ? 't-accent' : 't-danger';
    const level = total >= thresholds.high ? 0 : total >= thresholds.mid ? 1 : total >= thresholds.low ? 2 : 3;
    return (
      <div className="space-y-4">
        <div className="t-bg2 rounded-xl border t-border overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="t-bg3"><th className="text-left px-4 py-2">模块</th><th className="text-right px-4 py-2">得分</th><th className="text-left px-4 py-2">满分</th></tr></thead>
            <tbody>
              <tr className="border-t t-border"><td className="px-4 py-2">定性分析</td><td className="px-4 py-2 text-right font-bold">{sheet.qualTotal}</td><td className="px-4 py-2 t-muted">/40</td></tr>
              <tr className="border-t t-border"><td className="px-4 py-2">定量分析</td><td className="px-4 py-2 text-right font-bold">{sheet.quantScore}</td><td className="px-4 py-2 t-muted">/30</td></tr>
              <tr className="border-t t-border"><td className="px-4 py-2">{isFutures ? '风险收益比' : '估值与安全边际'}</td><td className="px-4 py-2 text-right font-bold">{sheet.valuationScore}</td><td className="px-4 py-2 t-muted">/20</td></tr>
              <tr className="border-t t-border">
                ...
                <td className="px-4 py-2 text-right"><input type="number" min="0" max="10" value={sheet.riskPlanScore || ''} onChange={e => save({ riskPlanScore: Math.min(10, Math.max(0, parseInt(e.target.value) || 0)) })}
                  className="t-input w-16 text-right font-bold" /></td>
                <td className="px-4 py-2 t-muted">/10</td>
              </tr>
              <tr className="border-t-2 t-border t-bg3"><td className="px-4 py-3 font-bold">总分</td><td className={`px-4 py-3 text-right text-2xl font-bold ${scoreColor}`}>{total}</td><td className="px-4 py-3 t-muted font-bold">/100</td></tr>
            </tbody>
          </table>
        </div>
        {sheet.vetoConclusion === 'failed' && <div className="p-3 bg-red-100 border border-red-200 rounded-lg text-sm t-danger font-medium">⛔ 注意：一票否决项未全部通过！</div>}
        ...
        <div className="space-y-3">
          <div><label className="text-sm font-medium t-text">是否投资</label>
            <div className="flex gap-2 mt-1">{['invest', 'watch', 'abandon'].map(d => (
              <button key={d} onClick={() => save({ decision: d as typeof sheet.decision })}
                className={`px-4 py-2 rounded-lg text-sm ${sheet.decision === d ? 't-accent-bg text-white' : 't-bg3 t-text2'}`}>
                {d === 'invest' ? '✅ 投资' : d === 'watch' ? '👀 继续观察' : '❌ 放弃'}
              </button>
            ))}</div></div>
          <div><label className="text-sm font-medium t-text">当前动作</label>
            <input value={sheet.decisionAction} onChange={e => save({ decisionAction: e.target.value })} placeholder="买入 / 定投 / 等待 / 放弃" className="w-full px-3 py-2 border t-border rounded-lg text-sm mt-1" /></div>
          <div><label className="text-sm font-medium t-text">核心原因（一句话）</label>
            <input value={sheet.decisionReason} onChange={e => save({ decisionReason: e.target.value })} className="w-full px-3 py-2 border t-border rounded-lg text-sm mt-1" /></div>
          <div className="flex gap-2">
            {(['completed', 'holding', 'abandoned'] as DecisionStatus[]).map(s => (
              <button key={s} onClick={() => save({ status: s })}
                className={`px-3 py-1.5 rounded-lg text-xs ${sheet.status === s ? StatusColor[s] + ' font-bold ring-2 ring-offset-1 ring-blue-400' : 't-bg3 t-text2'}`}>
                标记为: {StatusLabel[s]}
              </button>
            ))}
          </div>
  ...
  const renderTracking = () => (
    <div className="space-y-4">
      <button onClick={() => save({ trackingRecords: [...sheet.trackingRecords, { id: `tr_${Date.now()}`, date: new Date().toISOString().split('T')[0], event: '', affectsLogic: null, action: '', note: '' }] })}
        className="px-3 py-1.5 t-accent-bg text-white rounded-lg text-xs font-medium t-accent-bg-hover">+ 添加跟踪记录</button>
  ...
  const renderDiscipline = () => (
    <div className="space-y-3">
      <div className={`t-gradient rounded-xl p-5 text-white ${isCyber ? 'glow-border' : ''}`}>
  ...
  const renderQuick = () => {
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm t-text2">极简版快速初筛——用最少的问题做最核心的判断。</p>
        {questions.map((q, i) => (
          <div key={i} className="p-3 t-bg2 rounded-lg border t-border">
            <label className="text-sm font-medium t-text">{q}</label>
            <input value={sheet.valuationFields[`quick_${i}`] || ''} onChange={e => save({ valuationFields: { ...sheet.valuationFields, [`quick_${i}`]: e.target.value } })}
              className="t-input w-full text-sm mt-1" />
          </div>
        ))}
        <div className="p-3 t-bg2 rounded-lg border t-border">
          <label className="text-sm font-medium t-text">最终动作</label>
          <div className="flex gap-2 mt-1">{(isFutures ? ['开仓', '等', '放弃'] : sheet.assetType === 'fund' ? ['买', '定投', '等', '放弃'] : ['买', '等', '放弃']).map(a => (
            <button key={a} onClick={() => save({ valuationFields: { ...sheet.valuationFields, quick_action: a } })}
              className={`px-4 py-2 rounded-lg text-sm ${sheet.valuationFields.quick_action === a ? 't-accent-bg text-white' : 't-bg3 t-text2'}`}>{a}</button>
          ))}</div>
        </div>
      </div>
    );
